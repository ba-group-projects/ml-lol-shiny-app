...
createRf <- function(train, ntree) {
  ...
  # pred.train <- predict(rf, newdata = train[, -ncol(train)])
  # pred.train.acc <- round(mean(pred.train == train[, ncol(train)]) * 100, 1)

  # pred.test <- predict(rf, newdata = test[, -ncol(test)])
  # pred.test.acc <- round(mean(pred.test == test[, ncol(test)]) * 100, 1)

  # return(list(rf, pred.train.acc, pred.test.acc))
  return(rf)
}
...
server <- function(input, output, session) {
  ...
  output$rfPlot_ <- renderPlot(
    # rpart.plot(randomForest(), box.palette = "BuRd", roundint = FALSE)
